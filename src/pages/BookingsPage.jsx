...
const STATUSES = ['all', 'pending', 'confirmed', 'in_progress', 'completed', 'cancelled', 'disputed', 'refunded'];
const LIMIT = 20;
...
export default function BookingsPage() {
  ...
  useEffect(() => {
    const t = setTimeout(fetchBookings, search ? 400 : 0);
      ...
      case 'service':
        return (
          <div className="leading-tight">
            <p className="text-[#1A1A1A]">{b.service?.category || '—'}</p>
            {b.service?.location && <p className="text-[11.5px] text-gray-400">{b.service.location}</p>}
          </div>
        );
      case 'provider': return b.provider?.name || <span className="text-gray-400">Unassigned</span>;
      case 'payment':
        return (
          <div className="leading-tight text-right">
            <p className="font-medium text-[#1A1A1A] tabular-nums">{money(b.payment)}</p>
            <p className="text-[11.5px]"><StatusBadge status={b.payment?.status} /></p>
          </div>
        );
  ...
  return (
    <div className="p-6 max-w-[1280px] mx-auto space-y-6">
      <div>
        <h1 className="text-[20px] font-bold text-[#1A1A1A] tracking-tight">Bookings</h1>
        <p className="text-[13.5px] text-gray-500 mt-0.5">Monitor and manage service bookings across the SureLink platform</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total', value: stats.total ?? '—', dot: 'bg-gray-300', color: 'text-[#1A1A1A]' },
          { label: 'In Progress', value: stats.in_progress ?? '—', dot: 'bg-[#0057FF]', color: 'text-[#0057FF]' },
          { label: 'Disputed', value: stats.disputed ?? '—', dot: 'bg-[#FF6B00]', color: 'text-[#FF6B00]' },
          { label: 'Completed', value: stats.completed ?? '—', dot: 'bg-emerald-500', color: 'text-emerald-600' },
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div className="flex flex-col gap-3 border-b border-gray-100 p-4 md:flex-row md:items-center md:justify-between">
          <div className="relative w-full md:max-w-xs">
            ...
            <input
              ...
              placeholder="Search by reference, customer, provider…"
              value={search}
        ...
        <DataTable
          ...
          empty={<EmptyState icon="fa-calendar-check" title="No bookings found" message="Try adjusting your search or filters." />}
        />

        <Pagination page={page} limit={LIMIT} total={total} shown={bookings.length} onPageChange={setPage} />
      </div>
    </div>
  );
}
